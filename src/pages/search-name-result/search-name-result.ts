...
@Component({
  selector: 'page-search-name-result',
  templateUrl: 'search-name-result.html',
})
export class SearchNameResultPage {
  textEng: any = ["Search by Name"];
  textMyan: any = ["Search by Name"];
  searchKey: any = {"type": "", "companyName":""};;
  description: any; popover: any;
  textData: string[] = []; font: string = '';
  public isLoading; flag: any;
  constructor(private alertCtrl: AlertController, private all: AllserviceProvider, private toastCtrl: ToastController, private global: GlobalProvider, private nhttp: HTTP, private platform: Platform, private popoverCtrl: PopoverController, public navCtrl: NavController, public navParams: NavParams,
    ...
    this.searchByName(this.searchKey);
  ...
  searchByName(searchKey) {
    this.isLoading = 0;
    this.databaseprovider.searchByName(searchKey.type, searchKey.companyName).then(data => {
