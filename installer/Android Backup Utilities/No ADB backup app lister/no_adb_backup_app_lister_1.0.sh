...
# Description:
#
#	This program is a bash script that generates two lists containing
...
#	all the android packages which are present on the device, including
#	both system apps and those installed by the user.
#
...
#	and the other is in html format.
...
#	adb backup, you will notice that there are apps missing.
#	This issue has been reported to Google several times for more than 5 years, and still you
#	don't know exactly what and what not is included on your "backup" until you do a proper research.
#
# How it works:
...
echo "Computer date: $(date)<BR>" >> NO_ADB_BACKUP_PACKAGES.HTML
...
echo -n -e "It takes considerably longer (y/\033[4mn\033[0m) "
read answer
...
do
	...
	DUMPSYS_OF_CURENT_PACKAGE="$(adb shell dumpsys package "$CURRENT_PACKAGE_NAME")"
	adb shell dumpsys package "$CURRENT_PACKAGE_NAME" | grep -v '^$' > CURENT_PACKAGE_DUMPSYS.TXT
...
#	
	...
	if grep -q '^Hidden system packages:' CURENT_PACKAGE_DUMPSYS.TXT ; then
		#echo "$CURRENT_PACKAGE_NAME -> has hidden packages."
		CURRENT_PACKAGE_HAS_HIDDEN_PACKAGES="true"
		CURRENT_PACKAGE_STARTING_LINE_OF_HIDDEN_PACKAGES="$(grep -n -m 1 "^Hidden system packages:" CURENT_PACKAGE_DUMPSYS.TXT | awk -v FS="(:)" '{print $1}')"
#		CURRENT_PACKAGE_LINE_OF_HIDDEN_PACKAGES_FLAGS="$(sed -n '"$CURRENT_PACKAGE_STARTING_LINE_OF_HIDDEN_PACKAGES",$p' CURENT_PACKAGE_DUMPSYS.TXT | grep -m 1 ' *flags=\[')"
		CURRENT_PACKAGE_LINE_OF_HIDDEN_PACKAGES_FLAGS="$(tail -n +"$CURRENT_PACKAGE_STARTING_LINE_OF_HIDDEN_PACKAGES" CURENT_PACKAGE_DUMPSYS.TXT | grep -m 1 ' *flags=\[')"
	...
	then
		CURRENT_PACKAGE_MAIN_PACKAGES_STARTING_LINE="$(grep -n -m 1 "^Packages:" CURENT_PACKAGE_DUMPSYS.TXT | awk -v FS="(:)" '{print $1}')"
		CURRENT_PACKAGE_MAIN_PACKAGES_FLAGS_LINE="$(tail -n +"$CURRENT_PACKAGE_MAIN_PACKAGES_STARTING_LINE" CURENT_PACKAGE_DUMPSYS.TXT | grep -m 1 ' *flags=\[')"
	...
	then
		...
		then
			...
			adb pull "$CURRENT_PACKAGE_APK_PATH" CURRENT_APK.APK > /dev/null
			...
			#echo "Curent package label is: $CURRENT_PACKAGE_LABEL_LINE"
			...
			then
				...
				then
					...
					unzip -p CURRENT_APK.APK "$CURRENT_PACKAGE_ICON_RELATIVE_PATH" > "icons/$CURRENT_PACKAGE_ICON_NAME_PLUS_EXTENSION"
...
if [ -f CURENT_PACKAGE_DUMPSYS.TXT ]
then
	rm CURENT_PACKAGE_DUMPSYS.TXT
fi
...
echo "Job finished."
echo "Open \"ALL_PACKAGES.TXT\" and \"NO_ADB_BACKUP_PACKAGES.HTML\" for more details"
