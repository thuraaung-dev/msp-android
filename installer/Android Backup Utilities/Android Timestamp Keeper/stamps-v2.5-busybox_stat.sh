...
#	(cd $WDIR; find . -type f -exec sh -c 'busybox stat -c %n "{}" | sed -e "s/^\.\///g" ' \; -exec busybox stat -c %Y "{}" \; -exec sh -c 'busybox stat -c %y "{}" | sed -e 's/-//g' | sed -e 's/://g' | sed -e "s/ //g" | cut -d '.' -f1 | sed "s/./.&/13" ' \;) >> timestamps.txt
	...
	(cd "$WDIR"; find . -type f -exec sh -c 'busybox stat -c %n "{}" ' \; -exec busybox stat -c %Y "{}" \; -exec sh -c 'busybox stat -c %y "{}" | sed -e 's/-//g' | sed -e 's/://g' | sed -e "s/ //g" | cut -d '.' -f1 | sed "s/./.&/13" ' \;) >> timestamps.txt
